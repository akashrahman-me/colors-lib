function Intro() {
   return (
      <section className="mt-16 sm:mt-24 mb-14 sm:mb-20">
         <div className="container">
            <div className="flex gap-1.5 text-center flex-col max-w-[491px] mx-auto">
               <h1 className="font-semibold font-lexend-exa text-[32px] sm:text-4xl leading-normal">
                  Contrast Ratio
               </h1>
               <p className="font-medium text-[16px] sm:text-lg text-gray-700/75 leading-normal">
                  Measure the ratio between text and background colors for clear
                  readability.
               </p>
            </div>
         </div>
      </section>
   );
}

export default Intro;
